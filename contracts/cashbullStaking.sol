...
contract Staking {
    ...
    constructor(address token) {
        owner = (0xabc4B357D7419cfD3747DC1338e9e6308612D87c);
...
function changeOwner(address  _newOwner) external{
    ...
    require(_newOwner != address(0) && _newOwner !=address(0xdead),"New owner can't be null address");
    owner = _newOwner;
}
function withdrawTokens(address _token, uint256 _amount) external  {
    require(msg.sender== owner, "Only owner can call this function");
    require(_amount <= IERC20(_token).balanceOf(address(this)), "Insufficeint funds");
